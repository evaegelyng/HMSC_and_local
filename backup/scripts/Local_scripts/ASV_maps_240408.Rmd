...
# Add metadata
rich_t_97$cluster<-meta$cluster[match(row.names(rich_t_97),meta$root)]
rich_t_97$substrate_type<-meta$substrate_type[match(row.names(rich_t_97),meta$root)]

## Aggregate samples from the same cluster of the same substrate type, outputting read sum
rich_agg_97<-aggregate(. ~ cluster + substrate_type, rich_t_97, function(x) sum(x))
...
## Scatter plot for all data
p <- ggplot(wat_long, aes(x=sp_richness, y=asvs, colour="black")) +
    geom_point(size=2) +
    scale_y_log10() +
    labs(title="",
       x="Species richness", y = "ASV richness") +
    theme(legend.position = "none")
    #geom_abline(intercept=log10(A),slope=B)+
