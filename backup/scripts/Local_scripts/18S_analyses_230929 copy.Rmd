...
otus$sample<-factor(substr(row.names(otus),
                          1,
                          nchar(row.names(otus))-1))

#Check the number of biological replicates per sample
bio_reps<-otus %>% group_by(sample) %>% summarise(n=n())
#Select only samples with 3 replicates
three_reps<-filter(bio_reps,n=="3")
length(unique(otus$sample))
sp_18S_3<-otus[otus$sample %in% three_reps$sample, ]

# Aggregate replicate samples, taking the mean of read counts
rep_agg<-aggregate(. ~ sample, sp_18S_3, function(x) mean(x))
...
rep_agg$Substrate<-metadata$substrate_type[match(row.names(rep_agg),metadata$sample)]
...
rep_agg_pa$sc<-metadata$sc[match(row.names(rep_agg_pa),metadata$sample)]
...
rep_6hab$Substrate<-metadata$substrate_type[match(row.names(rep_6hab),metadata$sample)]
rep_6hab$habitat<-metadata$habitat[match(row.names(rep_6hab),metadata$sample)]
rep_6hab$season<-metadata$season[match(row.names(rep_6hab),metadata$sample)]
...
# Subset to clusters where all three habitats were sampled
sed$sc<-metadata$sc[match(row.names(sed),metadata$sample)]
...
sed_3hab$habitat<-metadata$habitat[match(row.names(sed_3hab),metadata$sample)]
...
# Subset to clusters where all three habitats were sampled
wat$sc<-metadata$sc[match(row.names(wat),metadata$sample)]
...
# Subset to each habitat
wat_3hab$habitat<-metadata$habitat[match(row.names(wat_3hab),metadata$sample)]
...
sed_3hab_new$habitat<-metadata$habitat[match(row.names(sed_3hab_new),metadata$sample)]
...
wat_3hab_new$habitat<-metadata$habitat[match(row.names(wat_3hab_new),metadata$sample)]
...
## Parameter settings inspired by https://rpubs.com/CPEL/NMDS
  ...
  site_scrs$New_Area <- metadata$New_Area[match(row.names(site_scrs), metadata$sample)]
...
## Parameter settings inspired by https://rpubs.com/CPEL/NMDS
  ...
  site_scrs_w$New_Area <- metadata$New_Area[match(row.names(site_scrs_w), metadata$sample)]
...
```{r nmds sed habitat}
  ...
  site_scrs$habitat <- metadata$habitat[match(row.names(site_scrs), metadata$sample)]
  site_scrs$season <- metadata$season[match(row.names(site_scrs), metadata$sample)]
...
```{r nmds wat habitat}
  ...
  site_scrs_w$habitat <- metadata$habitat[match(row.names(site_scrs_w), metadata$sample)]
  site_scrs_w$season <- metadata$season[match(row.names(site_scrs_w), metadata$sample)]
...
site_scrs$salinity <- metadata$salinity[match(row.names(site_scrs), metadata$sample)]
...
## Get salinities
site_scrs_w$salinity <- metadata$salinity[match(row.names(site_scrs_w), metadata$sample)]
...
## Parameter settings inspired by https://rpubs.com/CPEL/NMDS
  ...
  site_scrs <- as.data.frame(scores(allmds, display = "sites"))
  site_scrs$Substrate <- metadata$substrate_type[match(row.names(site_scrs), metadata$sample)]
