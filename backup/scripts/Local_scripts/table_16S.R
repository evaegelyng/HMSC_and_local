...
## Get tax table
taxa<-read.table("../Tekstfiler/16S/cleaned_tax_silva_EES.txt", sep="\t", row.names = 1, header=T)
...
## Get tax table
taxa_rare<-read.table("../Tekstfiler/16S/tax_rarefy_dada2.txt", sep="\t", row.names = 1, header=T)
...
## Get OTU table
otus1<-read.table("../Tekstfiler/16S/cleaned_otu_silva_EES.txt", sep="\t", header=T, row.names=1,check.names=F)

##Get metadata
metadata<-read.table("../Tekstfiler/16S/cleaned_silva_metadata_EES.txt", sep="\t", header=T)
...
## Write to file
write.table(n_otus,file="../Tekstfiler/16S/Table_16S.tsv",sep="\t")
