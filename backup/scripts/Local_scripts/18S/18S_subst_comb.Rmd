...
DT1<-readRDS("../../RDS/18S_no_c2_3reps.rds")
...
Richness looks a little skewed, but lets formally test it:
...
boxplot_habitat_season <- ggplot(dat, aes(x=habitat, y=rich, fill=habitat)) + 
  ...
  geom_text_repel(aes(label=cluster),na.rm=TRUE,nudge_y=0.05)+
  geom_text(data = counts, aes(x = habitat, y = 1700, label = paste0("n = ", n)), inherit.aes = FALSE)+
  geom_pwc(aes(group = habitat),
           tip.length = 0,
           method = "dunn_test")
...
saveRDS(boxplot_habitat_adj,"../../Plots/Boxplots/18S_habitat_comb.rds")
```
Looks like only the sand-rock comparison in autumn survives correction for multiple testing. 
...
saveRDS(nmds_sed,"../../Plots/NMDS/NMDS_18S_hab_comb.rds")
```
...
saveRDS(nmds_sal,"../../Plots/NMDS/NMDS_18S_sal_comb.rds")
```
