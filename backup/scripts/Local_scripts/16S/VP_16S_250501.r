...
var_Plot<- function(var_table,title=NULL,x_title=NULL,y_title=NULL,grouping=NULL,color_pallette=NULL){
  ...
  color_pallette<- color_pallette[order(summing$category,summing$sum)]
    ...
    for(i in 2:length(colnames(grouping))){
      ...
      var_table<- var_table[order(factor(var_table$kingdom,levels=kingdomOrder),var_table$phylum,var_table$clade,var_table$variables),]
...
model<- readRDS("../../RDS/fitTF_16S_sed.rds")
model_w<- readRDS("../../RDS/fitTF_16S_wat.rds")
...
taxonomy_w$kingdom<-taxa$Kingdom[match(taxonomy_w$class_name,taxa$Class)]
taxonomy_w$phylum<-taxa$Phylum[match(taxonomy_w$class_name,taxa$Class)]
...
ggsave("../../Plots/Var_par/16S_varpar_sed.png", plot = sed_plot, width=12, height=12)
...
ggsave("../../Plots/Var_par/16S_varpar_wat.png", plot = water_plot, width=12, height=12)
