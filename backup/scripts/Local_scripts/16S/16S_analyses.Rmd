...
ggplot(dat, aes(x=habitat, y=rich, fill=Substrate)) + 
  ...
  geom_text_repel(aes(label=cluster),na.rm=TRUE,nudge_y=0.05)+
  geom_signif(comparisons = list(c("eelgrass", "sand"), c("eelgrass", "rocks") ,c("sand", "rocks")), 
              map_signif_level=TRUE, test = t.test, step_increase = 0.05, tip_length = 0)
```
...
boxplot_habitat_adj <- ggadjust_pvalue(
  boxplot_habitat_season, p.adjust.method = "holm",
