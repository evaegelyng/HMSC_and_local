...
## Get phyloseq object
COSQ_no_c2 <- readRDS("../RDS/COSQ_rare2_correct_pident97.rds")

## Get OTU table
sp_97<-data.frame(otu_table(COSQ_no_c2),check.names=F)

## Get taxonomy
tax1<-data.frame(tax_table(COSQ_no_c2))

## Get metadata
sam1<-data.frame(sample_data(COSQ_no_c2))
...
sam1$sshc<-paste(sam1$substrate_type, sam1$season, sam1$habitat, sam1$cluster, sep="_")
##Including the number of field reps per sample
bio_reps<-sam1 %>% group_by(sshc) %>% summarise(n=n())
sam1$bio_reps<-bio_reps$n[match(sam1$sshc,bio_reps$sshc)]
```

```{r final filtering}

#Aggregate MOTUs with the same final ID
tax1<-as.data.frame(apply(tax1,c(1,2),function(x) gsub("/","_",x)))
sp_97$final.id<-tax1$final.id[match(row.names(sp_97),row.names(tax1))]
...
tax.new<-within(tax1,rm(id,species))
...
tax_mat_b<-as.matrix(tax.agg2)
OTU = otu_table(motu_agg, taxa_are_rows = TRUE)
TAX_b = tax_table(tax_mat_b)
p_NCBI = phyloseq(OTU, TAX_b)

##Add metadata
sampledata = sample_data(data.frame(sam1, row.names=sam1$root, stringsAsFactors=FALSE))
DADAwang1 = merge_phyloseq(p_NCBI, sampledata)
DADAwang1 

##Remove cluster 2, which was only sampled in one season
DADAwang2<-subset_samples(DADAwang1, !cluster==2)
DADAwang2 # 6 field reps removed

#Select only samples with 3 replicates
DADAwang3<-subset_samples(DADAwang2, bio_reps==3)
DADAwang3 # 22 field reps removed

# Merge field replicates
DT1<-merge_samples(DADAwang3, "sshc", fun = mean)
...
d$substrate_type<-sampledata$substrate_type[match(rownames(d),sampledata$sshc)]

sample_data(DT1)<-d[,c("sshc","sch","cluster","season","habitat","substrate_type")]

#Export phyloseq object as rds file
saveRDS(DT1,file="../RDS/COI_97_no_c2_3reps.rds")
...
## Get metadata
sam<-data.frame(sample_data(DT1))
```
...
sed$sc<-sam$sc[match(row.names(sed),sam$sshc)]
...
sed_3hab$habitat<-sam$habitat[match(row.names(sed_3hab),sam$sshc)]
...
# Subset to clusters where all three habitats were sampled
wat$sc<-sam$sc[match(row.names(wat),sam$sshc)]
...
# Subset to each habitat
wat_3hab$habitat<-sam$habitat[match(row.names(wat_3hab),sam$sshc)]
...
sed_3hab_t$family<-tax.agg2$family[match(row.names(sed_3hab_t),row.names(tax.agg2))]
...
sed_3hab_new$habitat<-sam$habitat[match(row.names(sed_3hab_new),sam$sshc)]
...
wat_3hab_t$family<-tax.agg2$family[match(row.names(wat_3hab_t),row.names(tax.agg2))]
...
wat_3hab_new$habitat<-sam$habitat[match(row.names(wat_3hab_new),sam$sshc)]
...
chrom<-tax.agg2[which(tax.agg2$kingdom=="Chromista"),]
plant<-tax.agg2[which(tax.agg2$kingdom=="Plantae"),]
metaz<-tax.agg2[which(tax.agg2$kingdom=="Metazoa"),]
...
sed_rs$cluster<-sam$cluster[match(row.names(sed_rs),sam$sshc)]
...
## Aggregate samples from the same cluster
wat_rs$cluster<-sam$cluster[match(row.names(wat_rs),sam$sshc)]
...
for (i in 1:length(datasets)){
  ...
  site_scrs$New_Area <- sam$New_Area[match(row.names(site_scrs), sam$sshc)]
  ...
  site_scrs$Community <- sam$sshc[match(row.names(site_scrs), sam$sshc)]
  site_scrs$cluster<-sam$cluster[match(row.names(site_scrs),sam$sshc)]
