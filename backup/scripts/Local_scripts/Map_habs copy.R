...
denmark_plot<-ggplot(denmark) + 
  ...
  geom_point(data = clusters_E, aes(x = coords.x1, y = coords.x2, color = "With eelgrass"), size = 7,alpha=0.5) + # Change color and size here
  geom_point(data = clusters_NE, aes(x = coords.x1, y = coords.x2, color = "Without eelgrass"), size = 7,alpha=0.5) + # Change color and size here
  ...
  scale_color_manual(name='',
                     breaks=c("With eelgrass","Without eelgrass"),
                     values=c("With eelgrass"="#1B9E77", 'Without eelgrass'="#E7298A", "Unused"="white"))+ 
  theme(legend.position = c(0.8, 0.8), text = element_text(size = 18))

ggsave("../Kort/Map_hab.png", denmark_plot, height = 16, width = 22, units = "cm")
