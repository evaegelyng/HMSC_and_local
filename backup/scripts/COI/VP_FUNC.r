...
var_Plot<- function(var_table,title=NULL,x_title=NULL,y_title=NULL,grouping=NULL,color_pallette=NULL){
  ...
  color_pallette<- color_pallette[order(summing$category,summing$sum)]
    ...
    for(i in 2:length(colnames(grouping))){
      ...
      var_table<- var_table[order(var_table$kingdom,var_table$phylum,var_table$class,var_table$variables),]
...
model<- readRDS("results/COI_rich_p50prevorder_water_plus_c2_thin10_s2000_tr02_notnorm.rds")


COSQ_rare2<-readRDS("results/COSQ_rare2_correct_score_tax.rds")
...
the_plot<- var_Plot(variance,title=NULL,y_title="Explained variance",grouping=tax, color_pallette=color_pallette_sed)
