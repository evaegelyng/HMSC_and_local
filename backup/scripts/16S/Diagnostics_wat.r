...
load(file = "Water/16S/unfitted_models.RData")

chain1 <- from_json(readRDS(file="Water/16S/post_file00.rds")[[1]])
chain2 <- from_json(readRDS(file="Water/16S/post_file01.rds")[[1]])
chain3 <- from_json(readRDS(file="Water/16S/post_file02.rds")[[1]])
chain4 <- from_json(readRDS(file="Water/16S/post_file03.rds")[[1]])
...
saveRDS(fitTF, file = "Water/16S/fitTF.rds")
...
text.file = file.path("Water/16S/MCMC_convergence.txt")
...
nm = length(models)
pdf(file= file.path("Water/16S/MCMC_convergence.pdf"))
if(showBeta){
  par(mfrow=c(2,1))
...
save(MF,MFCV,WAIC,file = "Water/16S/model_fit.Rdata")